     ...
     <form action="{{ route('schools.employee.update') }}" id="updateEmployeeForm" method="POST" enctype="multipart/form-data">
         ...
         <div class="flex flex-col items-center justify-center gap-2">
             <div class="w-full text-center">
                 ...
                 <div class="page-title">Editing Employee Information</div>
                 <div class="page-subtitle">Review the following information</div>
             </div>
         </div>
         ...
         <div class="grid md:grid-cols-3 grid-cols-1 gap-4 mb-4">
             ...
             <div class="md:col-span-3 col-span-1">
                 <div class="relative inline-block group">
                     ...
                     <div class="shadow-md p-1 border border-gray-300 rounded-full">
                         <img id="editSchoolPreview" class="md:w-40 md:h-40 h-20 w-20 rounded-full object-cover"
                             src="{{ asset('icon/profile.png') }}" alt="Profile Photo">
                     </div>


                     <div id="camera-button-edit"
                         ...
                         <button type="button" onclick="document.getElementById('schoolImageEdit').click()"
                     ...
                     <!-- Hidden File Input -->
                     <input type="file" name="image_path" id="schoolImageEdit" class="hidden" accept="image/*">

                     <!-- Action Buttons -->
                     <div id="imageActionsEdit" class="hidden mt-3 flex justify-center gap-2">
                 ...
                 <script>
                     let originalSrcEdit = document.getElementById('editSchoolPreview').src;

                     document.getElementById('schoolImageEdit').addEventListener('change', function(e) {
                         selectedFile = e.target.files[0];
                         if (!selectedFile) return;

                         const reader = new FileReader();
                         reader.onload = function(e) {
                             document.getElementById('editSchoolPreview').src = e.target.result;
                         };
                         reader.readAsDataURL(selectedFile);

                         document.getElementById('imageActionsEdit').classList.remove('hidden');
                         document.getElementById('camera-button-edit').classList.add('hidden');
                     });

                     function cancelImage() {
                         document.getElementById('editSchoolPreview').src = originalSrcEdit;
                         document.getElementById('schoolImageEdit').value = '';
                         document.getElementById('imageActionsEdit').classList.add('hidden');
                         selectedFile = null;
                         document.getElementById('camera-button-edit').classList.remove('hidden');
             ...
             <x-input-field type="text" name="fname" label="First Name" :required="true" :edit="true" />
             <x-input-field type="text" name="mname" label="Middle Name" :required="false" :edit="true" />
             <x-input-field type="text" name="lname" label="Last Name" :required="true" :edit="true" />
             ...
             <!-- Sex & Birthdate -->
             <x-select-field name="sex" label="Sex" :options="collect([['id' => 'M', 'name' => 'Male'], ['id' => 'F', 'name' => 'Female']])" :required="true" :edit="true"
                 valueField="id" textField="name" />

             <x-input-field type="date" name="birthdate" label="Birthdate" :required="true" :edit="true" />
             ...
             <x-select-field name="m365_email_status" label="m365 Email Status" :options="collect([['id' => 'Active', 'name' => 'Active'], ['id' => 'Inactive', 'name' => 'Inactive']])" :edit="true"
             ...
             <x-select-field name="canva_login_status" label="Canva Login Status" :options="collect([['id' => 'Active', 'name' => 'Active'], ['id' => 'Inactive', 'name' => 'Inactive']])"
                 :edit="true" :required="true" valueField="id" textField="name" />
             <x-select-field name="lr_portal_status" label="LR Portal Status" :options="collect([['id' => 'Active', 'name' => 'Active'], ['id' => 'Inactive', 'name' => 'Inactive']])" :edit="true"
                 :required="true" valueField="id" textField="name" />
             <!-- Recipients -->
             <x-select-field name="l4t_recipient" label="L4T Recipient" :options="collect([['id' => 'Yes', 'name' => 'Yes'], ['id' => 'No', 'name' => 'No']])" :edit="true"
                 :required="true" valueField="id" textField="name" />
             <x-select-field name="smart_tv_recipient" label="Smart TV Recipient" :options="collect([['id' => 'Yes', 'name' => 'Yes'], ['id' => 'No', 'name' => 'No']])"
                 :edit="true" :required="true" valueField="id" textField="name" />
             <x-select-field name="l4nt_recipient" label="L4NT Recipient" :options="collect([['id' => 'Yes', 'name' => 'Yes'], ['id' => 'No', 'name' => 'No']])" :edit="true"
                 :required="true" valueField="id" textField="name" />
             <div class="md:col-span-3 col-span-1 text-lg font-semibold ">
             ...
             <x-select-field name="officer_in_charge" label="Is Officer in Charge?" :options="collect([['id' => '1', 'name' => 'Yes'], ['id' => '0', 'name' => 'No']])"
                 :edit="true" :required="true" valueField="id" textField="name" />
             <!-- Other Fields -->
             <x-select-field name="inactive" label="Is Employee Inactive?" :options="collect([['id' => '1', 'name' => 'Yes'], ['id' => '0', 'name' => 'No']])" :edit="true"
                 :required="true" valueField="id" textField="name" />
