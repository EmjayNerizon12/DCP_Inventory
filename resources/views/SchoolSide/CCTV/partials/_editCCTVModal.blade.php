<x-modal id="edit-cctv-modal" size="large-modal" type="edit" icon="cctv-lg">
        <form action="{{ route('schools.equipment.update') }}" id="updateCCTVForm" class="space-y-4" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid md:grid-cols-2 grid-cols-1 gap-2">
                ...
                <x-select-field name="e_cctv_type" label="Camera Type" :options="$cctv_type" :required="true" :edit="true" valueField="pk_e_cctv_camera_type_id" textField="name" />
            ...
            <div class="modal-button-container">
                <button type="button" onclick="closeComponentModal('edit-cctv-modal')"
                    class="btn-cancel px-6 py-1 md:w-auto sm:w-fit w-full rounded">
                    Cancel
                </button>
                <button type="submit" id="updateCCTVButton" class="btn-green sm:w-fit w-full px-4 py-1 rounded">
                    Update Details
                </button>
            </div>
...
<script>
    const updateCCTVForm = document.getElementById('updateCCTVForm');
    const updateCCTVButton = document.getElementById('updateCCTVButton');
    updateCCTVForm.addEventListener('submit', async (e) =>{
        e.preventDefault();
        buttonLoading(updateCCTVButton);

        const formData = new FormData(updateCCTVForm);
        formData.append('_method','PUT');
        const response = await fetch(updateCCTVForm.action,{
            method: 'POST',
            headers:{
        ...
        if(!response.ok){
            ...
            resetButton(updateCCTVButton, 'Update Details');
            return;
        }
        closeComponentModal('edit-cctv-modal');
        updateCCTVForm.reset(); 
